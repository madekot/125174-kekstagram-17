...
(function () {
  ...
  var DEFAULT_SLIDER_POSITION = 100;
  ...
  var FILTER_MAX_VALUES = [1, 1, 100, 3, 3];
  ...
  var resetPosition = function (position) {
    position = position || DEFAULT_SLIDER_POSITION;
  ...
  var convertClassToFilterStyle = function (effectValue) {
    ...
    switch (imagePreviewElement.className) {
      case FILTER_CLASSES[1]: result = FILTER_NAMES[0] + '(' + convertPercentToFilterValue(effectValue, FILTER_MAX_VALUES[0]) + ')';
        break;
      case FILTER_CLASSES[2]: result = FILTER_NAMES[1] + '(' + convertPercentToFilterValue(effectValue, FILTER_MAX_VALUES[1]) + ')';
        break;
      case FILTER_CLASSES[3]: result = FILTER_NAMES[2] + '(' + convertPercentToFilterValue(effectValue, FILTER_MAX_VALUES[2]) + '%' + ')';
        break;
      case FILTER_CLASSES[4]: result = FILTER_NAMES[3] + '(' + convertPercentToFilterValue(effectValue, FILTER_MAX_VALUES[3]) + 'px' + ')';
        break;
      case FILTER_CLASSES[5]: result = FILTER_NAMES[4] + '(' + convertPercentToFilterValue(effectValue, FILTER_MAX_VALUES[4], FILTER_HEAT_MIN_VALUE) + ')';
        break;
    }
    return result;
  };
  ...
  var effectValue = DEFAULT_SLIDER_POSITION;
  ...
  var addClassAddChangeEvent = function (imageFilterPreviewElement, filterClass) {
    imageFilterPreviewElement.addEventListener('click', function () {
      ...
      resetPosition(DEFAULT_SLIDER_POSITION);
